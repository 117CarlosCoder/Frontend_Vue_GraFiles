    ...
    >
      ...
      <v-card-actions v-if="isImageType(file.fileType)" :class="'bg-warning'" >
        ...
        <v-menu offset-y>
          ...
          <template v-if="file">
            <ViewDialog @change="fetchFiles" :file="file" />
          </template>
        </v-menu>
      </v-card-actions>
      <v-card-actions v-else-if="file.fileType" :class="'bg-primary'" >
        ...
        <v-menu offset-y>
          ...
          <template v-if="file">
            <ViewDialog @change="fetchFilesChange" :file="file" />
          </template>
        </v-menu>
      </v-card-actions>
  ...
  export default {
    ...
    setup() {
      ...
      const fetchFiles = async () => {
        ...
        try {
          const response = await axios.get(`http://localhost:8080/admin/getDirectorys?id=${directoryId.value}`, { withCredentials: true });
          ...
          const responsefile = await axios.get(`http://localhost:8080/admin/getFiles?id=${directoryId.value}`, { withCredentials: true });

          const responseSharesfiles= await axios.get(`http://localhost:8080/admin/getSharedFiles?id=${directoryId.value}`, { withCredentials: true });
      ...
      const routeChange = async () => {
        ...
        try {
          const response = await axios.get(`http://localhost:8080/admin/getDirectorys?id=${routeSave.value.pop()}`, { withCredentials: true });
          files.value = response.data;
  
           const responsefile = await axios.get(`http://localhost:8080/admin/getFiles?id=${fileActual.value.id}`, { withCredentials: true });
